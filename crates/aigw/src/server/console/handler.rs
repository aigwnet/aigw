...
use aigw_core::{AcmeToken, ChangeLog, Cluster, DataFrame, LogAction, LogType, Site};
use tokio::fs;
...
impl DataFrameHandler {
    ...
    pub async fn handle(&self, item: &DataFrame) -> anyhow::Result<bool> {
        for change_log in &item.logs {
            // A poisoned log must not abort the whole frame: isolate the error,
            // keep applying the remaining logs, and always persist the log point
            // below, otherwise the console resends the frame forever and later
            // logs starve.
            if let Err(e) = self.apply_change_log(change_log).await {
                error!(target: "console", "Apply change log {} ({:?}/{:?}) error: {:?}",
                    change_log.log_id, change_log.log_type, change_log.log_action, e);
            }
        }
    ...
    async fn apply_change_log(&self, change_log: &ChangeLog) -> anyhow::Result<()> {
