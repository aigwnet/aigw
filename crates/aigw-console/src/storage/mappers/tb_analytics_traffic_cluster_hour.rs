...
impl TbAnalyticsTrafficClusterHour {
    ...
    pub async fn insert<'e, E: MySqlExecutor<'e>>(
        e: E,
        table: &TbAnalyticsTrafficClusterHour,
    ...
    pub async fn select_by_cluster<'e, E: MySqlExecutor<'e>>(
        ...
        cluster_name: &str,
        limit: u64,
    ) -> sqlx::Result<Vec<TbAnalyticsTrafficClusterHour>> {
        sqlx::query_as::<_, TbAnalyticsTrafficClusterHour>(
            "SELECT * FROM tb_analytics_traffic_cluster_hour WHERE cluster_name = ? ORDER BY id DESC LIMIT ?",
        )
        .bind(cluster_name)
        .bind(limit)
        .fetch_all(e)
    ...
    ) -> sqlx::Result<Option<TbAnalyticsTrafficClusterHour>> {
        sqlx::query_as::<_, TbAnalyticsTrafficClusterHour>(
            "SELECT * FROM tb_analytics_traffic_cluster_hour WHERE cluster_name = ? and gmt_create = ?",
        )
        .bind(cluster_name)
        .bind(gmt_create)
        .fetch_optional(e)
    ...
    pub async fn select_by_cluster_gmt_create_greater<'e, E: MySqlExecutor<'e>>(
        e: E,
        cluster_name: &str,
        gmt_create: OffsetDateTime,
    ) -> sqlx::Result<Vec<TbAnalyticsTrafficClusterHour>> {
    ...
    pub async fn select_page_by_cluster_and_time(
    ...
    ) -> sqlx::Result<DbPage<TbAnalyticsTrafficClusterHour>> {
        let mut page = DbPage {
            records: vec![],
            total: 0,
        ...
        if page_request.do_count {
            let (total,): (i64,) = sqlx::query_as(
                "SELECT count(1) FROM tb_analytics_traffic_cluster_hour WHERE cluster_name = ? and gmt_create >= ? and gmt_create < ?",
            )
            .bind(cluster_name)
            .bind(start_time)
            .bind(end_time)
        ...
        page.records = sqlx::query_as::<_, TbAnalyticsTrafficClusterHour>(
            "SELECT * FROM tb_analytics_traffic_cluster_hour WHERE cluster_name = ? and gmt_create >= ? and gmt_create < ? ORDER BY id DESC LIMIT ?,?",
        )
        .bind(cluster_name)
        .bind(start_time)
        .bind(end_time)
